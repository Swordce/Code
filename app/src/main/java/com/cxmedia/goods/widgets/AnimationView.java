...
import com.cxmedia.goods.utils.ImageUtils;
...
public class AnimationView extends View implements Handler.Callback {
    ...
    private void startPlay() {
        ...
        try {
            if ( mAnimDataList != null && mAnimDataList.size() > 0 ){
                ...
                mCurShowBmp = ImageUtils.getBitmap(getContext(),animData.filePath,mWidth,mHeight);
                invalidate();
                if (mListener != null ){
            ...
            Toast.makeText(getContext(),"动画开始失败",Toast.LENGTH_SHORT).show();
        }
    ...
    private void checkIsPlayNext() {
        try {
            ...
            Toast.makeText(getContext(),"播放下一帧失败" + e.getMessage(),Toast.LENGTH_SHORT).show();
        }
                ...
                public boolean handleMessage(Message msg) {
                    ...
                    switch (msg.what){
                        case PROCESS_DATA:{
                            ...
                            Bitmap bitmap = ImageUtils.getBitmap(mContext.get(),animData.filePath,mWidth,mHeight);
                            if (bitmap != null ){
